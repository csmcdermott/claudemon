...
- **Chart tooltip date titles (2026-07-02)**: `tooltipDateTitle(ts, range)` in app.js returns `toLocaleDateString(undefined,{month:'short',day:'numeric'})` (e.g. `Jul 11`) for day views, and `${fmtHour(ts)} · ${md}` (e.g. `8am · Jul 2`) for hour views. Wired as the `title` callback in all three render functions, resolving `ts` from `padded[items[0].dataIndex].date`. The **x-axis tick labels are unchanged** (`bucketLabel` still returns terse `11`/`8am`) — only the tooltip title is enriched. The token chart sets ONLY a `title` callback; Chart.js v4 merges its default `label` callback so the body rows (Output/Input/Cache %) still render — verified live.
