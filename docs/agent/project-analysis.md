...
| **Last updated** | 2026-07-02 (v0.8.2: self-update install fix — move-aside instead of in-place `ditto` overwrite of the running bundle; follows v0.8.0 axis-label date/time hover chip + WKWebView locale-safe date formatting) |
...
| Updater | claudemon/updater.py | GitHub release check (1h cache, `_UPDATE_LOCK`), version compare, `perform_update` (non-daemon thread, URL allowlist, `# pragma: no cover`) + `_install_app` (move-aside install, testable) |
...
- **updater.py (2026-06-16)**: `check_for_updates()` hits `https://api.github.com/repos/csmcdermott/claudemon/releases/latest` with 1h cache (`CACHE_TTL = 3_600`, `_UPDATE_LOCK`). `asset_url` is stored in cache internally but **never returned in any HTTP response** — it's only accessible via `get_update_asset_url()` used by `POST /api/update`. `get_update_state_for_response()` strips `asset_url` for HTTP responses. `perform_update` is `# pragma: no cover`, validates URL netloc against `_TRUSTED_HOSTS = {"github.com", "objects.githubusercontent.com"}`, uses `tempfile.TemporaryDirectory()` as context manager, `ditto` for extract, `_install_app` for install, `shutil.rmtree` before SIGTERM. Runs in a **non-daemon thread** so it completes even if the main thread exits. **Install must not overwrite the target bundle in place (2026-07-02, v0.8.2)**: during self-update the app runs *from* `/Applications/claudemon.app`, and macOS refuses to overwrite the running, memory-mapped executable (`ETXTBSY`) → bare "exit status 1". `_install_app(src, dest)` renames the old bundle aside (`os.rename` is safe on a running app — same inode keeps the mmap valid), `ditto`s a fresh copy into place, restores the backup on failure, and captures `ditto` stderr into the raised error. It's factored out of `perform_update` specifically so it's unit-testable (subprocess mocked, real tmp dirs).
...
- **Query names (2026-07-02)**: Queries are IDed `sessionShortId:taskNum:queryNum` but now also carry a human name. Captured at index time from the first user prompt of each query (`_extract_prompt_text`, joins all text blocks), **truncated to 60 chars before storage** in the `queries` table (`upsert_query`, INSERT OR IGNORE so the first prompt wins). `query_query_breakdown` LEFT JOINs it in as `text` (None if the query's start message had no text). The query chart tooltip shows `q.text || q.query_id`, display-truncated to ~50 chars. **No `esc()`** on the query text — it renders only in the Chart.js canvas tooltip, not `innerHTML`.
- **Chart tooltip date titles (2026-07-02)**: `tooltipDateTitle(ts, range)` in app.js returns `fmtMonthDay(ts)` (e.g. `Jul 11`) for day views, and `${fmtHour(ts)} · ${md}` (e.g. `8am · Jul 2`) for hour views. Wired as the `title` callback in all three render functions, resolving `ts` from `padded[items[0].dataIndex].date`. The **x-axis tick labels are unchanged** (`bucketLabel` still returns terse `11`/`8am`) — only the tooltip title is enriched. The token chart sets ONLY a `title` callback; Chart.js v4 merges its default `label` callback so the body rows (Output/Input/Cache %) still render — verified live. **Date formatting is locale-independent** (`fmtMonthDay` = `MONTH_ABBR[getMonth()]` + `getDate()`): `toLocaleDateString` misbehaves in WKWebView the same way `toLocaleTimeString` does (see lessons), returning an empty title that made Chart.js render no title line — the original v0.7.x tooltip date never appeared in the app.
- **Axis-label date/time hover chip (2026-07-02, v0.8.0)**: Chart.js shows no tooltip when hovering x-axis tick labels, and its `onHover` callback does not even fire below `chartArea.bottom`. So `attachAxisHover(chart)` (called for all three charts in `initCharts`) adds a **native** `canvas.addEventListener('mousemove'/'mouseleave')`. On mousemove, if the cursor is in the axis-label strip (`y > chartArea.bottom` and `x` within `[left,right]`), it maps x→index via `chart.scales.x.getValueForPixel(x)`, resolves the timestamp via `bucketTsForIndex(idx)` (shared with `onChartClick`: `_paddedTimeline[idx]?.date ?? _paddedTasks[idx]?.date ?? _paddedQueries[idx]?.date`), and shows a `position:fixed` `#axis-tip` chip with `tooltipDateTitle(ts, currentRange)` at the cursor. Hides on bar hover / leaving the canvas. Bar/line tooltips are untouched (whole-column `mode:'index'` was tried and rejected — it aggregated all series). DOM/CSS chip chosen because native `title` and Chart.js hover are both unreliable here.
- **Skill/MCP hover tooltip (2026-07-02)**: The native `title=""` attribute (unreliable in WKWebView — see lessons) was replaced by a custom CSS tooltip. `.tool-row` is `position: relative`; a hidden `.tool-tip` span (last child of the row, name wrapped in `esc()`) is revealed by `.tool-row:hover .tool-tip { visibility: visible }` — so hovering **anywhere on the row** (name, bar, or meta) shows the full name, not just the ellipsis.

## Recently Changed Areas

| Date | File / Area | What changed |
| --- | --- | --- |
| 2026-07-02 | v0.8.2 (self-update install fix) | `perform_update` no longer overwrites `/Applications/claudemon.app` in place (failed with `ETXTBSY` on the running executable → "exit status 1"). New `_install_app(src, dest)` renames old bundle aside → `ditto` fresh copy → restore on failure; captures `ditto` stderr. Released as GitHub v0.8.2. **Old installs must be updated manually once** — their old updater still fails. |
| 2026-07-02 | claudemon/updater.py | Added `_install_app(src, dest)` (move-aside install, `capture_output` for stderr); `perform_update` now calls it instead of the in-place install `ditto` |
| 2026-07-02 | tests/test_updater.py | +4 tests for `_install_app`: fresh install, replace-existing, move-aside-before-ditto, restore-on-failure (subprocess mocked, real tmp dirs) |
| 2026-07-02 | v0.8.0 (axis-label date/time hover) | New `#axis-tip` chip shows the full date/time when hovering the x-axis label strip on all three charts (native canvas mousemove, not Chart.js hover). Also fixed the v0.7.x tooltip date: `toLocaleDateString` → locale-safe `fmtMonthDay` (was invisible in WKWebView). |
| 2026-07-02 | dashboard/app.js | `fmtMonthDay()` + `MONTH_ABBR` (replace `toLocaleDateString` in `tooltipDateTitle` and the `bucketLabel` midnight-crossing branch); `bucketTsForIndex()` helper (reused by `onChartClick`); `attachAxisHover(chart)` native mousemove/mouseleave listener; called for all three charts in `initCharts` |
| 2026-07-02 | dashboard/index.html | Added `<div id="axis-tip"></div>` before the app.js script |
| 2026-07-02 | dashboard/style.css | Added `#axis-tip` (position:fixed, hidden by default, tooltip styling) |
| 2026-07-02 | v0.7.x (dashboard hover improvements) | Three features: query names from first prompt, richer chart tooltip date/time titles, whole-row skill/MCP hover tooltip |
| 2026-07-02 | claudemon/db.py | New `queries(query_id PK, session_id, text)` table in SCHEMA; `upsert_query()` (INSERT OR IGNORE, first-write-wins, holds `_LOCK`); `query_query_breakdown` now `LEFT JOIN queries` → each per-query dict gains a `text` field (None when absent) |
| 2026-07-02 | claudemon/indexer.py | `_QUERY_TEXT_MAX = 60`; `_extract_prompt_text()` (joins all text blocks, whitespace-collapsed); new-query branch calls `db.upsert_query(...)` with text truncated to 60 chars **at index time** (full prompts never stored) |
| 2026-07-02 | dashboard/app.js | `tooltipDateTitle(ts, range)` helper (`Jul 11` day / `8am · Jul 2` hour); title callback set in `renderTokenChart`/`renderQueryChart`/`renderTaskChart`; query tooltip label shows `q.text` (fallback `query_id`), display-truncated ~50; `renderSkills`/`renderMcp` drop `title=""`, add `<span class="tool-tip">` |
| 2026-07-02 | dashboard/style.css | `.tool-row { position: relative }`; `.tool-tip` (hidden, absolute above row) + `.tool-row:hover .tool-tip { visibility: visible }` |
